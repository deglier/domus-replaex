...
var _prepareDestination = _interopRequireWildcard(__webpack_require__(8695));
...
function prepareDestination(destination, params, query, appendParamsToQuery) {
    ...
    const destPathCompiler = pathToRegexp.compile(destPath, // have already validated before we got to this point and validating
    ...
    for (const [key1, strOrArray] of Object.entries(destQuery)){
        ...
        if (Array.isArray(strOrArray)) {
            destQuery[key1] = strOrArray.map((value)=>compileNonPath(unescapeSegments(value), params)
            );
        } else {
            destQuery[key1] = compileNonPath(unescapeSegments(strOrArray), params);
        }
    }
